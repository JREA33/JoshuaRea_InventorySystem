...
namespace JoshuaRea_InventorySystem
{
    public partial class ModifyPartScreen : Form
    {
        ...
        //Initialize an InHouse Part

        public ModifyPartScreen(InHousePart inHousePart)
        {
            InitializeComponent();

            TxtPartID = inHousePart.PartID;
            TxtName =   inHousePart.Name;
            TxtInventory = inHousePart.InStock;
            TxtPrice = inHousePart.Price;
            TxtMax = inHousePart.Max;
            TxtMin = inHousePart.Min;
            TxtMachineID = inHousePart.MachineID;

            rbInHouse.Checked = true;
        }

        //Initialize an Outsourced Part

        public ModifyPartScreen(OutsourcedPart outsourcedPart)
        {
            InitializeComponent();

            TxtPartID = outsourcedPart.PartID;
            TxtName = outsourcedPart.Name;
            TxtInventory = outsourcedPart.InStock;
            TxtPrice = outsourcedPart.Price;
            TxtMax = outsourcedPart.Max;
            TxtMin = outsourcedPart.Min;
            TxtCompanyName = outsourcedPart.CompanyName;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (rbInHouse.Checked == true)
                {
                    ...
                    Inventory.updatePart(TxtPartID, inHousePart);
                }
                if (rbOutsourced.Checked == true)
                {
                    ...
                    Inventory.updatePart(TxtPartID, outsourcedPart);
                }
                this.Close();
